...
- **Self-hosting** — Hecks generates itself from its own Bluebook chapters. All 16 chapters (Bluebook, Kernel, Runtime, Binding, CLI, Extensions, Targets, Workshop, Hecksagon, AI, Rails, Spec, Templating, Watchers, Persist, Examples) boot as running Hecks applications via `InMemoryLoader` + `Runtime`. Convention-driven auto-discovery: `require_paragraphs` (eager) + `define_paragraphs` (lazy) replaces manual registration.
- **Kernel chapter** — Descriptive Bluebook chapter covering the bootstrap infrastructure (DSL builders, domain model IR, registries, core utilities, chapter system). The interpreted runtime still uses `require_relative` for these files (chicken-and-egg), but the chapter definitions prove the Bluebook covers 100% of the framework. The compiled binary will use these definitions for chapter-dispatch loading.
