...
- **Self-hosting** — Hecks generates itself from its own Bluebook chapters. All 15 chapters (Bluebook, Runtime, Binding, CLI, Extensions, Targets, Workshop, Hecksagon, AI, Rails, Spec, Templating, Watchers, Persist, Examples) boot as running Hecks applications via `InMemoryLoader` + `Runtime`. 670 aggregates, 836 commands, 836 events. Convention-driven auto-discovery: `require_paragraphs` (eager) + `define_paragraphs` (lazy) replaces manual registration.
