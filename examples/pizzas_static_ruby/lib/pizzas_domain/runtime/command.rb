  ...
  module Runtime
    ...
    module Command
      ...
      def then
        ...
        begin
          result = yield(self)
          if result.is_a?(self.class) || (result.class.ancestors.include?(PizzasDomain::Runtime::Command))
      ...
      def check_preconditions
        self.class.preconditions.each do |cond|
          unless instance_exec(&cond.block)
            raise PizzasDomain::PreconditionError, "Precondition failed: #{cond.message}"
          end
        end
      end
      ...
      def check_postconditions(before, after)
        self.class.postconditions.each do |cond|
          unless cond.block.call(before, after)
            raise PizzasDomain::PostconditionError, "Postcondition failed: #{cond.message}"
          end
        end
      end
      ...
      def run_guard
        ...
        unless result
          raise PizzasDomain::GuardRejected, "Guard #{policy_name} rejected #{self.class.name.split('::').last}"
        end
      end
