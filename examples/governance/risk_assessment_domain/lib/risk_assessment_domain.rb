...
module RiskAssessmentDomain
  ...
  autoload :Assessment, "risk_assessment_domain/assessment/assessment"

  module Ports
    autoload :AssessmentRepository, "risk_assessment_domain/ports/assessment_repository"
  end

  module Adapters
    autoload :AssessmentMemoryRepository, "risk_assessment_domain/adapters/assessment_memory_repository"
  end
end
...
if defined?(Hecks) && Gem.loaded_specs["risk_assessment_domain"]
  _hecks_domain_file = File.join(Gem.loaded_specs["risk_assessment_domain"].full_gem_path, "hecks_domain.rb")
  if File.exist?(_hecks_domain_file)
    Kernel.load(_hecks_domain_file)
    RiskAssessmentDomain.instance_variable_set(:@_hecks_domain, Hecks.last_domain)
  end

  RiskAssessmentDomain.define_singleton_method(:boot) do |**opts, &block|
  ...
  RiskAssessmentDomain.define_singleton_method(:runtime) { @runtime }

  RiskAssessmentDomain.boot unless ENV["HECKS_SKIP_BOOT"]
end
