    ...
    module Commands
      class InvestigateIncident
        ...
        def call
          ...
          if existing
            unless existing.status == "reported"
              raise OperationsDomain::Error, "Cannot InvestigateIncident: status must be 'reported', got '#{existing.status}'"
            end
            Incident.new(
              id: existing.id,
