...
RSpec.describe Hecks::Workbench::AggregateHandle do
  let(:workbench) { Hecks::Workbench.new("Pizzas") }

  before { allow($stdout).to receive(:puts) }

  describe "getting a handle" do
    it "returns a handle from workbench.aggregate" do
  ...
  describe "#attr" do
    ...
    it "supports reference_to" do
      workbench.aggregate("Pizza")
      order = workbench.aggregate("Order")
  ...
  describe "#verb" do
    it "adds a custom verb to the workbench" do
      pizza = workbench.aggregate("Pizza")
      pizza.verb("Poop")
      domain = workbench.to_domain
  ...
  describe "implicit dot syntax" do
    ...
    it "returns a CommandHandle for chained attribute additions" do
      pizza = workbench.aggregate("Pizza")
      handle = pizza.create
      expect(handle).to be_a(Hecks::Workbench::CommandHandle)
    end
