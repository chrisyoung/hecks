...
module Hecks
  # Hecks::McpServer
  #
  # MCP (Model Context Protocol) server that exposes the Hecks Workbench API
  ...
  #   - +session+ accessor for the current Hecks::Workbench
  ...
  #
  class McpServer
    # The current Hecks::Workbench instance, set after +create_session+ or
    # +load_domain+ tool is called. +nil+ until a session is established.
    #
    # @return [Hecks::Workbench, nil]
    ...
    # tool handlers before accessing +@workbench+.
    #
    # @raise [RuntimeError] if +@workbench+ is nil
    # @return [void]
    def ensure_session!
      raise "No session. Call create_session first." unless @workbench
    end
