    ...
    module PlayTools
      ...
      def self.register(server, ctx)
        server.define_tool(
          ...
          ctx.ensure_session!
          ctx.capture_output { ctx.workbench.play! }
        end
        ...
        server.define_tool(
          ...
          ctx.ensure_session!
          ctx.capture_output { ctx.workbench.sketch! }
        end
        ...
        server.define_tool(
          ...
          ctx.capture_output { ctx.workbench.execute(args["command"], **attrs) }
        end
        ...
        server.define_tool(
          ...
          ctx.ensure_session!
          ctx.capture_output { ctx.workbench.history }
        end
        ...
        server.define_tool(
          ...
          ctx.ensure_session!
          ctx.capture_output { ctx.workbench.extend(args["name"].to_sym) }
        end
      end
    end
  end
