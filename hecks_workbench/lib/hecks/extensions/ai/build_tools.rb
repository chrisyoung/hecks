    ...
    module BuildTools
      ...
      def self.register(server, ctx)
        server.define_tool(
          ...
          ctx.ensure_session!
          ctx.capture_output { ctx.workbench.validate }
        end
        ...
        server.define_tool(
          ...
          ctx.ensure_session!
          ctx.capture_output { ctx.workbench.build(output_dir: args["output_dir"] || ".") }
        end
        ...
        server.define_tool(
          ...
          ctx.ensure_session!
          ctx.capture_output { ctx.workbench.save(args["path"] || "hecks_domain.rb") }
        end
        ...
        server.define_tool(
          ...
          ctx.ensure_session!
          domain = ctx.workbench.send(:to_domain)
          require "hecks_serve"
          port = args["port"] || 9292
        ...
        server.define_tool(
          ...
          ctx.ensure_session!
          ctx.capture_output { ctx.workbench.promote(args["aggregate"]) }
        end
      end
    end
  end
