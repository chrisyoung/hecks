module Hecks
  module MCP
    ...
    # can be used. These tools set +ctx.workbench+ on the shared McpServer context.
    #
    # Registered tools:
    ...
    module SessionTools
      ...
      # @param ctx [Hecks::McpServer] shared context; +ctx.workbench+ set on invoke
      # @return [void]
      def self.register(server, ctx)
        server.define_tool(
          ...
          ctx.workbench = Hecks.workbench(args["name"])
        ...
        server.define_tool(
          ...
          ctx.capture_output do
            ...
            ctx.workbench = Hecks.workbench(domain.name)
            domain.aggregates.each do |agg|
              handle = ctx.workbench.aggregate(agg.name)
