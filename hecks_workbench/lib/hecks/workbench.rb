require "fileutils"
require_relative "workbench/build_actions"
require_relative "workbench/play_mode"
require_relative "workbench/presenter"
require_relative "workbench/handles/aggregate_handle"
require_relative "workbench/system_browser"
...
module Hecks
  # Hecks::Workbench
  #
  # Interactive domain-building workbench for REPL-driven development. Supports
  ...
  # each wrapped in an AggregateHandle for interactive use. The workbench can
  ...
  #
  #   workbench = Hecks.workbench("Pizzas")
  ...
  class Workbench
    ...
    # Initialize a new workbench for the given domain name.
    #
    ...
    # Check whether the workbench is in sketch (build) mode.
    #
    # @return [Boolean] true when the workbench is in sketch mode
    def sketch?
      @mode == :sketch
    end

    # Check whether the workbench is in play (runtime) mode.
    #
    # @return [Boolean] true when the workbench is in play mode
    def play?
      @mode == :play
    end
    ...
    def aggregate(name, &block)
      ...
      handle = @handles[name] ||= AggregateHandle.new(name, builder, domain_module: Names.domain_module_name(@name), workbench: self)
    ...
    def active_hecks!
      ...
      puts "ActiveHecks loaded for #{Names.domain_module_name(domain.name)}"
      mod
    end

    # Check if ActiveHecks has been activated for this workbench.
    #
    ...
    # Return the list of aggregate names defined in this workbench.
    #
    ...
    # Remove an aggregate by name from the workbench.
    #
    ...
    #
    # Extracts the named aggregate from this workbench, creates a new domain
    # containing just that aggregate, writes it to a file, and removes it
    # from the current workbench. The new domain can be wired back via
    # `extend NewDomain` for cross-domain event listening.
    #
    #   workbench.promote("Comments")
