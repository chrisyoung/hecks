module RustProjection
  module Projector
    ...
    # args_struct:, creates:}], entity_commands: [{verb:, entity_record:,
    # fn:, args_struct:}]}]` — accumulated by domain_generator.rb while it
    # walks the real IR, not re-derived here.
    def emit_registry(domain_name, aggregates)
        ...
        a[:commands].map do |c|
          ...
          <<~RUST.rstrip
                    #{c[:verb].inspect} => {
                        ...
                        #{dispatch_call}.map(|(_, events)| stamp_payload(events, args_json))
                    }
          RUST
        end
      end
      ...
      entity_arms = aggregates.flat_map do |a|
        a[:entity_commands].map do |c|
          <<~RUST.rstrip
                    #{c[:verb].inspect} => {
                        let parent_id = super::#{a[:mod]}::#{a[:record]}::extract_id(args_json)?;
                        let element_id = super::#{a[:mod]}::#{c[:entity_record]}::extract_id(args_json)?;
                        let args = super::#{a[:mod]}::#{c[:args_struct]}::from_json(args_json)?;
                        super::#{a[:mod]}::dispatch_entity_#{c[:fn]}(&mut store.#{a[:mod]}, &parent_id, &element_id, args).map(|(_, events)| stamp_payload(events, args_json))
                    }
          RUST
        end
      end
