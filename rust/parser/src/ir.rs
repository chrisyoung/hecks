...
/// `IR::PortOperation#to_h` — `direction` ALWAYS on the wire, `answers`
/// and `refuses` only when present, which is exactly the Ruby shape:
/// `shape[:answers] = @answers if @answers`.
///
/// The two directions are not symmetric in what they carry. `tells` is the
/// outside calling in and names `emits`; `asks` is the domain calling out
/// and names both endings it can live with, so its `emits` is empty and its
/// `answers`/`refuses` are not.
#[derive(Debug, Clone, Default)]
