...
fn port_operation_json(op: &ir::PortOperation) -> JsonValue {
    // KEY ORDER IS THE CONTRACT — this is compared byte-for-byte against
    // Ruby`s own JSON.pretty_generate, so the order here is `to_h``s order:
    // name, attributes, emits, direction, then the two optional endings.
    let mut fields = vec![
        ...
        ("direction".to_string(), JsonValue::str(op.direction.clone())),
    ];
    if let Some(answers) = &op.answers {
        fields.push(("answers".to_string(), JsonValue::str(answers.clone())));
    }
    if let Some(refuses) = &op.refuses {
        fields.push(("refuses".to_string(), JsonValue::str(refuses.clone())));
    }
