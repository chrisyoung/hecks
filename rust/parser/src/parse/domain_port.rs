...
pub fn parse_body(file: &str, lines: &[SourceLine], pos: &mut usize, name: &str, owner: Option<&str>) -> ParseResult<ir::DomainPort> {
    ...
    loop {
        ...
        match gated.row.word {
            // THREE SPELLINGS, TWO DIRECTIONS. `operation` is the original
            // word and is `tells` under an older name — answered forever, the
            // way every renamed word in this language is. `tells` is the
            // outside calling IN; `asks` is the domain calling OUT.
            "operation" | "tells" => {
                let op_name = super::positional_text(file, line, gated.row.word, &gated.args, 1)?;
                port.operations.push(parse_operation_body(file, lines, pos, &op_name, owner, "inbound")?);
            }
            "asks" => {
                let op_name = super::positional_text(file, line, "asks", &gated.args, 1)?;
                port.operations.push(parse_operation_body(file, lines, pos, &op_name, owner, "outbound")?);
            }
...
/// here").
fn parse_operation_body(file: &str, lines: &[SourceLine], pos: &mut usize, name: &str, owner: Option<&str>, direction: &str) -> ParseResult<ir::PortOperation> {
    ...
    loop {
        ...
        match gated.row.word {
            ...
            "answers" => operation.answers = Some(super::positional_text(file, line, "answers", &gated.args, 1)?),
            "refuses" => operation.refuses = Some(super::positional_text(file, line, "refuses", &gated.args, 1)?),
            "reference_to" => {
...
fn validate_operation(file: &str, line: usize, operation: &ir::PortOperation, owner: Option<&str>) -> ParseResult<()> {
    // AN ASK IS JUDGED ON ITS TWO ENDINGS, A TELL ON ITS `emits`. Both rules
    // say the same thing — an operation with nothing to say afterward is a
    // call into nothing — and `PortOperationBuilder#build` splits them the
    // same way.
    if operation.direction == "outbound" {
        if operation.answers.is_none() || operation.refuses.is_none() {
            return Err(Diagnostic::new(file, line, format!("'{}' does not name both endings — an ask says what it answers and what it refuses", operation.name)));
        }
    } else if operation.emits.is_empty() {
