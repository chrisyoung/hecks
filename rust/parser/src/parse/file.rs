...
pub fn parse_header<'a>(file: &str, lines: &[SourceLine], pos: &mut usize) -> ParseResult<(&'a KeywordRow, Call, usize)> {
    let line = *lines
        .get(*pos)
        .ok_or_else(|| Diagnostic::new(file, 0, "empty file — expected bluebook/hecksagon/world"))?;
    let shape = lex::classify(file, &line)?;
    *pos += 1;

    match shape {
        LineShape::End => Err(Diagnostic::new(file, line.number, "unexpected 'end' at the top of the file")),
        LineShape::Call(call) => {
