...
//! near-miss of a word the language declares at aggregate-body level,
//! whether or not it opens a block — refuses at boot rather than
//! booting under a domain definition the reference implementation
...
#[test]
fn a_near_miss_word_that_opens_no_block_also_refuses_at_boot() {
    // The first pass at this scan only ever looked at lines ending in `do`, so
    // a typo of a word that never opens a block — `attribute` chief among them
    // — fell through the same hole the block-keyword check was built to close.
    // The scan now checks every non-blank, non-comment line at aggregate-body
    // depth zero, not only the ones that open one.
    let root = std::env::temp_dir().join(format!("hecks-strict-nonblock-{}", std::process::id()));
    let _ = fs::remove_dir_all(&root);
    let bluebook_dir = root.join("bluebook");
    fs::create_dir_all(&bluebook_dir).unwrap();
    fs::write(
    ...
    assert_eq!(
        refusal,
        "cannot boot Typo::Sketch: this runtime could not read 'atribute' (did you mean 'attribute'?) — \
         a construct it cannot read refuses rather than half-reads; \
         host-language strictness is partial until the specializer"
    );
