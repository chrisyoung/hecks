...
RSpec.describe Hecksagain::BluebookIR do
  ...
  it "refuses to emit a construct that never declared its shape" do
    silent = Class.new { include Hecksagain::BluebookIR }

    expect { silent.new.to_h }.to raise_error(Hecksagain::BluebookIR::Undeclared, /never declared its shape/)
  end
  ...
  describe "the declaration is readable" do
    it "reports the emitted shape of a real construct" do
      ...
      expect(spec[:attributes]).to be_a(Hecksagain::BluebookIR::Many)
      expect(spec[:lifecycle]).to be_a(Hecksagain::BluebookIR::One)
    end
  end
end
