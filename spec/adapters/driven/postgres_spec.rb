...
#
# `pg` is required here explicitly — the adapter itself only requires it
# lazily, inside `Postgres.connect_for` (a domain that never wires
# Postgres should never need the gem), so this probe needs its own copy
# before it can reference `PG` at all.
require "pg"
