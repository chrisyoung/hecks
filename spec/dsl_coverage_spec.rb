...
RSpec.describe "the DSL surface is fully covered" do
  ...
  COVERED = {
    ...
    "DomainPortBuilder"           => [
      ...
      # directly-defined method at all. `verb`/`signal` — the bare-Port
      # fallback shape, ordinary defs mirroring PortBuilder's own (see
      # that class's own row, above).
      %i[tells_impl asks_impl verb signal]
    ],
