...
#
# `lib/hecksagain/projections/` is the sanctioned home for a "canonical
# IR in, external artifact out" tool (`Projector::Target`'s own doc
# comment, lib/hecksagain/projector/target.rb) — every file there is
...
RSpec.describe "the seam between canonical IR and its projections (ADR 0027)" do
  PROJECTIONS_DIR = File.join(InMemoryDomain::ROOT, "lib/hecksagain/projections")

  it "registers every file in lib/hecksagain/projections/ as a live Projector target" do
    files = Dir.glob(File.join(PROJECTIONS_DIR, "*.rb"))
    expect(files).not_to be_empty,
                         "lib/hecksagain/projections/ is empty or missing — this spec's own path is stale"
    ...
    findings = files.filter_map do |path|
      ...
      next if Hecksagain::Projector.registered?(key)

      "#{File.basename(path)} declares projects_as :#{key}, but Hecksagain::Projector doesn't have it " \
        "live — registered: #{Hecksagain::Projector.registered.sort.inspect}"
    end
  ...
  # construct that genuinely fits one of `lib/hecksagain/projector.rb`'s
  ...
  KNOWN_NON_PROJECTIONS = {
    "lib/hecksagain/projector/exporter.rb" =>
