...
RSpec.describe Hecks::DomainConnections do
  ...
  describe "#extend with persistence" do
    it "stores unnamed adapter under :default" do
      mod.extend(:sqlite)
      expect(mod.connections[:persist][:default]).to eq(Hecks::PersistConfig.new(type: :sqlite))
    end

    it "accepts additional options" do
      mod.extend(:sqlite, database: "test.db")
      expect(mod.connections[:persist][:default]).to eq(Hecks::PersistConfig.new(type: :sqlite, database: "test.db"))
    end
  end
  ...
  describe "#extend with outbound handler" do
    ...
    it "treats symbol + kwargs as outbound when not persistence" do
      ...
      expect(entry.options[:webhook]).to eq("https://hooks.slack.com/x")
    end
  end
  ...
  describe "#extend with middleware" do
    it "records a middleware extension" do
      mod.extend(:tenancy)
      expect(mod.connections[:extensions].map(&:name)).to include(:tenancy)
    end
  end
