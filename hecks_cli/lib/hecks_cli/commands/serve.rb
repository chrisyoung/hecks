...
# With --standalone, builds a standalone gem and serves with the built-in UI.
#
#   hecks domain serve [--domain NAME] [--port 9292] [--rpc] [--standalone]
#
module Hecks
  class CLI < Thor
    class Domain < Thor
      ...
      option :standalone, type: :boolean, default: false, desc: "Serve with standalone UI"
      ...
      def serve
        ...
        if options[:standalone]
          serve_standalone(domain, options[:port])
        elsif options[:rpc]
          require "hecks_serve"
      ...
      def serve_standalone(domain, port)
        require "tmpdir"
        dir = Dir.mktmpdir("hecks-serve-")
        output = Hecks.build_standalone(domain, output_dir: dir)
        lib_path = File.join(output, "lib")
        $LOAD_PATH.unshift(lib_path)
        gem_name = domain.gem_name
        require gem_name
        mod = Object.const_get(domain.module_name + "Domain")
        say "Serving #{mod.name} (standalone) on http://localhost:#{port}", :green
        mod.serve(port: port)
      ensure
