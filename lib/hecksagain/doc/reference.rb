...
module Hecksagain
  module Doc
    ...
    module Reference
      ...
      def tool_table(root)
        scripts = (Dir.glob(File.join(root, "bin/*")) + domain_clis(root))
                  .select { |path| File.file?(path) }.uniq.sort
        rows = scripts.filter_map { |path| [path, tool_summary(path)] }.select { |_, desc| desc }
        lines = ["| tool | |", "|---|---|"]
        rows.each { |path, desc| lines << "| `#{path.delete_prefix("#{root}/")}` | #{desc} |" }
        lines.join("\n")
      end

      # A DOMAIN'S COMMAND LINE IS A TOOL, WHEREVER IT LIVES.
      #
      # This table used to be `bin/*` and nothing else, which was the same
      # thing as the truth for as long as every projected launcher was minted
      # into `bin/`. They live beside their domains now — `qa/quality_control`,
      # `examples/pizzas/pizzas` — so the old glob would have quietly dropped
      # three rows and left the README claiming the repository has no domain
      # CLIs at all. A generated document that goes silently incomplete is
      # worse than one that was never generated.
      #
      # FOUND BY THE MARKER THEY CARRY rather than by naming the domains: every
      # one is written by `bin/project_cli` and says so on line three, so this
      # needs no list to keep in step and no domain has to be booted to build
      # a table of contents.
      def domain_clis(root)
        Dir.glob(File.join(root, "**/*.hecksagon"))
           .map { |path| File.dirname(File.dirname(path)) }
           .uniq
           .flat_map { |domain| Dir.glob(File.join(domain, "*")) }
           .select { |path| File.file?(path) && File.executable?(path) }
           .select { |path| File.foreach(path).first(4).any? { |line| line.include?("GENERATED by bin/project_cli") } }
      end
