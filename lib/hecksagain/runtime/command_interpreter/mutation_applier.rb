  ...
  module Runtime
    class CommandInterpreter
      ...
      module MutationApplier
        ...
        # COERCED AS AN ELEMENT, NOT AS THE LIST, which is the whole subtlety.
        #
        # `mutation.target` is a `list_of`, so handing the search value to
        # `Value.for_attribute` under that attribute asks it to build a LIST —
        # and it obliges, wrapping the one value into `[value]`. Comparing each
        # element against a one-element array then matches nothing, silently:
        # `remove` answered the untouched list and looked like it had worked.
        #
        # It read correctly until `hydrate_value_list` started coercing
        # value-object lists at all. Before that, a `list_of(<value object>)`
        # was passed through raw, so `for_attribute` handed the bare hash
        # straight back and hash-to-hash comparison happened to line up. The
        # comparison was always against the wrong SHAPE; it was only ever right
        # because neither side was being coerced.
        #
        # Passing `[value]` and taking `.first` runs the same coercion the
        # stored elements went through — pattern, invariants and all — so the
        # two sides are like-shaped by construction rather than by luck, and
        # `Value#==` compares type name and fields.
        def removed(instance, aggregate, mutation, args)
          value     = @rules.resolve_source(mutation.source, args)
          attribute = aggregate.attribute(mutation.target)
          value     = coerce_element(aggregate, attribute, value) if attribute
          Array(instance[mutation.target]).reject { |element| element == value }
        end

        def coerce_element(aggregate, attribute, value)
          return Value.for_attribute(aggregate, attribute, value) unless attribute.list?

          Array(Value.for_attribute(aggregate, attribute, [value])).first
        end
