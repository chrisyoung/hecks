...
module Hecksagain
  module Adapters
    class Lambda
      ...
      # involved at all.
      #
      # FUNCTION NAME IS COMPUTED, NOT PASSED — `"hecksagain-#{domain}"`,
      # lowercased, matches bin/project_deploy's own `stack_name`
      # exactly (bin/project_deploy: `stack_name = "hecksagain-#{domain_name}"`,
      ...
      class Client
        def initialize(domain:, region:)
          ...
          @function_name = "hecksagain-#{domain.to_s.downcase}"
