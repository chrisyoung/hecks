      ...
      def generate_aggregates(root, gem_name, mod)
        @domain.contexts.each do |ctx|
          ...
          ctx.aggregates.each do |agg|
            ...
            agg_gen = AggregateGenerator.new(agg, domain_module: mod, context_module: ctx_mod)
            autoload_gen = AutoloadGenerator.new(@domain)
            ...
            agg.value_objects.each do |vo|
              vo_gen = ValueObjectGenerator.new(vo, domain_module: mod, aggregate_name: agg.name, context_module: ctx_mod)
              write_file(root, "#{base}/#{Hecks::Utils.underscore(vo.name)}.rb", vo_gen.generate)
            end

            # Commands
            agg.commands.each do |cmd|
              cmd_gen = CommandGenerator.new(cmd, domain_module: mod, aggregate_name: agg.name, context_module: ctx_mod)
              write_file(root, "#{base}/commands/#{Hecks::Utils.underscore(cmd.name)}.rb", cmd_gen.generate)
            end

            # Events
            agg.events.each do |evt|
              evt_gen = EventGenerator.new(evt, domain_module: mod, aggregate_name: agg.name, context_module: ctx_mod)
              write_file(root, "#{base}/events/#{Hecks::Utils.underscore(evt.name)}.rb", evt_gen.generate)
            end

            # Policies
            agg.policies.each do |pol|
              pol_gen = PolicyGenerator.new(pol, domain_module: mod, aggregate_name: agg.name, context_module: ctx_mod)
