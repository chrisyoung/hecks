...
(function () {
  ...
  function setupActions() {
    document.addEventListener("click", function (e) {
      ...
      switch (action) {
        ...
        case "expand-sidebar":
          window.HecksApp.handleEvent({
            event: "SidebarToggled",
            data: { sidebar_collapsed: !window.HecksApp.getState().layout.sidebarCollapsed }
          });
          break;
        case "toggle-events":
          window.HecksApp.handleEvent({
            event: "EventsPanelToggled",
            data: { events_collapsed: !window.HecksApp.getState().layout.eventsCollapsed }
          });
          break;
        case "pause-stream":
          cmd("EventStream", "PauseStream", {});
          break;
        case "clear-events":
          cmd("EventStream", "ClearEvents", {});
          break;
      }
    });
  }
  ...
  function setupTabs() {
    document.addEventListener("click", function (e) {
      ...
      if (!tab) return;
      window.HecksIDE.command("Layout", "SelectTab", { tab: tab.dataset.tab });
    });
  }
