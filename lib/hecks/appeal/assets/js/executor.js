...
(function () {
  ...
  function appendMessage(agent, role, content, toolsHtml) {
    ...
    container.insertAdjacentHTML("beforeend",
      ...
      nameTag + escapeHtml(content) + (toolsHtml || "") + "</div>" +
      tapbacks + reactions +
      "</div></div>"
    );
  ...
  function handleEvent(data) {
    if (data.type === "executor_message") {
      ...
      if (data.tools_used && data.tools_used.length > 0) {
        toolsHtml = data.tools_used.map(function(t) {
          return '<div class="text-[10px] text-white/[.24] font-mono mt-1">' +
            '\u2192 ' + escapeHtml(t.name) + (t.input ? ' ' + escapeHtml(t.input) : '') + '</div>';
        }).join("");
      }
      appendMessage(data.agent, data.role || "assistant", data.content, toolsHtml);
