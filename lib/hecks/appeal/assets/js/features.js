      ...
      if (actionBtn) {
        ...
        if (action === "plan") {
          ...
          sendToBuilder("plan", { title: feature.title, description: feature.description }, function(result) {
            if (result.additions) {
              handleFeatureEvent({ event: "FeaturePlanned", data: { feature_id: id, additions: result.additions } });
            }
          });
        }
        if (action === "build") {
          ...
          sendToBuilder("build", { additions: feature.additions }, function(result) {
            if (result.file) {
              handleFeatureEvent({ event: "FeaturePlanned", data: { feature_id: id, additions: feature.additions } });
              ...
              setTimeout(function() {
                sendToBuilder("verify", { additions: feature.additions }, function(vr) {
                  ...
                  if (vr.complete) handleFeatureEvent({ event: "FeatureCompleted", data: { feature_id: id } });
                });
              }, 1000);
            }
          });
        ...
        if (action === "verify") {
          sendToBuilder("verify", { additions: feature.additions }, function(result) {
            ...
            if (result.complete) handleFeatureEvent({ event: "FeatureCompleted", data: { feature_id: id } });
          });
        }
        return;
      }
