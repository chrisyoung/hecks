...
(function () {
  ...
  function captureFrame() {
    ...
    html2canvas(target, {
      ...
      if (base64) {
        var dispatch = window.Hecks ? window.Hecks.dispatch : (window.HecksIDE && window.HecksIDE.command);
        if (dispatch) dispatch("Screenshot", "CaptureScreen", { frame_data: base64, timestamp: new Date().toISOString() });
      }
      busy = false;
    }).catch(function (err) {
