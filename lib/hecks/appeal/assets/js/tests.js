...
(function () {
  ...
  function runNext(tests, idx) {
    if (idx >= tests.length) {
      running = false;
      renderSummary();
      setTimeout(function () { fireEvent("TabSelected", { tab_name: "tests" }); }, 300);
      return;
    }
    ...
    setTimeout(function () {
      ...
      results.push({ command: test.name, event: evt, status: got ? "passed" : "pending" });
  ...
  function renderResults() {
    ...
    c.innerHTML = results.map(function (r) {
      ...
      var cls = r.status === "passed" ? "text-green-400" : "text-white/[.38]";
      var evt = r.event ? " → " + r.event : "";
      return '<div class="flex items-center gap-2 py-0.5 text-xs ' + cls + '"><span>' + icon + '</span><span class="font-mono">' + r.command + '</span><span class="text-white/[.38]">' + evt + '</span></div>';
    }).join("");
