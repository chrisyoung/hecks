...
(function () {
  ...
  var STORAGE_KEY = "hecks_appeal_state";

  function saveState() {
    try {
      var toSave = {
        layout: state.layout,
        agent: { mode: state.agent.mode, messages: state.agent.messages },
        editor: state.editor
      };
  ...
  function restoreState() {
    try {
      ...
      if (parsed.agent) Object.assign(state.agent, parsed.agent);
      if (parsed.editor) Object.assign(state.editor, parsed.editor);
      fullRender();
    } catch (e) {}
  }
