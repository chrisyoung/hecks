...
(function () {
  ...
  function setMode(newMode) {
    ...
    if (input) {
      input.placeholder = mode === "sketch"
        ...
        : "Aggregate.Command args or 'help'";
    }
  ...
  function showModeGuide() {
    var docs = window.Hecks && window.Hecks.docs;
    if (!docs) return;

    clearOutput();

    if (mode === "play") {
      appendOutput("info", "Play Mode — execute commands against the live domain");
      appendOutput("hint", "");
      appendOutput("hint", "Commands dispatch through Hecks.dispatch().");
      appendOutput("hint", "Client-side commands run instantly. Server commands go over WebSocket.");
      appendOutput("hint", "");
      if (docs.play && docs.play.length > 0) {
        appendOutput("info", "Available commands:");
        docs.play.forEach(function (c) {
          var line = "  " + c.cmd;
          if (c.desc) line += "  — " + c.desc;
          appendOutput("event", line);
        });
      }
      if (docs.sketch && docs.sketch.length > 0) {
        appendOutput("hint", "");
        appendOutput("info", "Client-side (instant):");
        docs.sketch.forEach(function (c) {
          appendOutput("event", "  " + c.cmd);
        });
      }
    } else {
      appendOutput("info", "Sketch Mode — inspect and author domain structure");
      appendOutput("hint", "");
      appendOutput("hint", "View your domain, explore aggregates, and run UI commands.");
      appendOutput("hint", "");
      if (docs.sketch && docs.sketch.length > 0) {
        appendOutput("info", "UI commands (instant):");
        docs.sketch.forEach(function (c) {
          var line = "  " + c.cmd;
          if (c.desc) line += "  — " + c.desc;
          appendOutput("event", line);
    ...
    appendOutput("hint", "");
    appendOutput("hint", "Type 'help' for syntax, 'list' for all commands, 'clear' to reset.");
  }
  ...
  function init() {
    setupModeToggle();
    setupInput();
    // Show guide after a short delay so Hecks.docs is loaded
    setTimeout(showModeGuide, 100);
  }
