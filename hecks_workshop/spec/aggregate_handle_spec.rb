...
RSpec.describe Hecks::Workshop::AggregateHandle do
  let(:workshop) { Hecks::Workshop.new("Pizzas") }

  before { allow($stdout).to receive(:puts) }

  describe "getting a handle" do
    it "returns a handle from workshop.aggregate" do
  ...
  describe "#verb" do
    it "adds a custom verb to the workshop" do
      pizza = workshop.aggregate("Pizza")
      pizza.verb("Poop")
      domain = workshop.to_domain
  ...
  describe "implicit dot syntax" do
    ...
    it "returns a CommandHandle for chained attribute additions" do
      pizza = workshop.aggregate("Pizza")
      handle = pizza.create
      expect(handle).to be_a(Hecks::Workshop::CommandHandle)
    end
