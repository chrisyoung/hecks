...
pub fn update_synapses(ctx: &DaemonCtx, carrying: &str, now: &str) -> f64 {
...
pub fn fire_signals(ctx: &DaemonCtx, concept: &str, beat: i64, strength: f64) {
...
pub fn consolidate_signals(ctx: &DaemonCtx, now: &str) {
    ...
    sorted.sort_by_key(|(_, s)| {
        s.get("created_at").and_then(|v| v.as_str()).unwrap_or("").to_string()
    });
...
pub fn update_impulses(ctx: &DaemonCtx, concept: Option<&str>) {
...
pub fn update_executive(ctx: &DaemonCtx, carrying: &str, concept: Option<&str>, now: &str) {
    if carrying == "—" { return; }
...
pub fn compost_synapses(ctx: &DaemonCtx, now: &str) {
    let path = ctx.store("synapse");
    let mut store = heki::read(&path).unwrap_or_default();
...
pub fn update_singletons(ctx: &DaemonCtx, carrying: &str, concept: Option<&str>, now: &str) {
    let mut focus = Record::new();
    focus.insert("target".into(), Value::String(carrying.into()));
    focus.insert("weight".into(), serde_json::json!(
        if concept.is_some() { 1.0 } else { 0.5 }));
