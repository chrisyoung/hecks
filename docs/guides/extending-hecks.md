...
— pizzas, an orchard, a bank. `lib/hecksagain/language/bluebook/*.bluebook`
...
row before it is a line of Ruby.** You do not start in a builder file.
You start in `syntax.bluebook`, and everything downstream — the
builder, the docs, the generated reference page — answers to that row,
not the other way around.

```ruby
Hecksagain::Bluebook::MetaValidator.grammar_registry.bluebook("Bluebook").aggregates.map(&:hecks_name).sort
...
like any other, judged the same way, with no commands of its own
because nothing dispatches it. A domain does not declare its own
syntax; the syntax is fixed when the language is defined, and this is
where it is fixed. The same rows this section just read are what
`bin/reference` projects into `docs/reference/*.md` — the prose you
...
1. **Snapshot** the two files the change can touch —
   `syntax.bluebook` and the golden `spec/golden/ir/Bluebook.json` —
   byte for byte, in memory, before doing anything.
2. **Rewrite the row** — text surgery on `syntax.bluebook`, not a
...
```ruby
(Hecksagain::Bluebook::DSL::TranslationAggregateBuilder.public_instance_methods(false) - [:build, :method_missing]).sort
...
changes the emitted `Syntax`/`Argument` rows, which is exactly the kind
...
`lib/hecksagain/bluebook/model_check.rb`, and the shape is the same for
...
`lib/hecksagain/doc/reference.rb` projects `Syntax::Keyword` and
`Syntax::Argument` into one Markdown page per context —
`docs/reference/aggregate.md`, `docs/reference/command.md`, and so on —
...
to satisfy it, and `lib/hecksagain/grammar/expression.bluebook` — one
...
is that same insurance policy aimed at the table you now know how to
edit — before it existed, the very `Syntax` aggregate this guide walked
