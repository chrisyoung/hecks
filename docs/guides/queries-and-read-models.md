...
guide runs against the real `examples/banking/bluebook/banking.bluebook`,
loaded directly rather than invented for this page — the same file
`docs/guides/verification.md` and every adapter spec in this repo
...
```ruby skip
# Customer, in examples/banking/bluebook/banking.bluebook
...
```ruby skip
# CardPayment, in examples/banking/bluebook/banking.bluebook
query "Flagged" do
...
```ruby skip
# SafeDepositBox, in examples/banking/bluebook/banking.bluebook
query "Rented" do
...
seal_c.call   # ~> Malformed: declares no floor attribute
```
...
```ruby skip
# Banking, in examples/banking/bluebook/banking.bluebook
