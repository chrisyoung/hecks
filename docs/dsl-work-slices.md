...
| **S6 — events first-class** | S0a, S0b | **Investigated 2026-08-27, genuinely the biggest remaining piece — scoped, not built.** Confirmed already done: command-side references (`trigger Account::Debit`, saga `dispatch`) resolve as bare constants via `ConstShim`, refuse quoted form outside `shadow_parse` (`policy_builder.rb`'s `trigger_impl`). Confirmed still open: `emits(event_name)` (`command_builder.rb:322`) stores a bare string, no attribute declaration; `on`/policy triggers still quoted, in two inconsistent spellings live today (qualified `"Account.AccountFrozen"` vs bare `"CustomerSuspended"`); `Runtime::Event`'s payload (`command_rules/emission.rb:18`) is the command's whole raw `args`, nothing to check `with:` against. No `Event` DSL builder exists anywhere (zero grep hits). **Design recommendation, not yet built:** (1) new `event "Name" do attribute :x, Type ... end` block inside `aggregate`, mirroring `ValueObjectBuilder`'s shape (same `AttributeCollector`/`WordGate` mixins); (2) `emits` references the declared event by bare constant, payload built from the command's own args the way `value_object` construction already works; (3) `on`/trigger(event-side)/saga-transition triggers resolve via `ConstShim`, same mechanism `trigger_impl` already proves; (4) `with:` checked against the event's declared attributes at meta-validator time — the actual point of the slice; (5) ~100+ corpus `emits`/`on` sites plus Rust parser/codegen are the largest remaining cost, budgeted as their own pass. Attempting the full scope in one agent pass was judged too risky (self-hosting/doc-generation/golden-IR gates are load-bearing on grammar completeness) — correctly stopped rather than land a half-broken tree. |
