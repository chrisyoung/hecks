...
| slice | note |
|---|---|
| **S14 — `Status` becomes a lifecycle** | **Still blocked — on a NARROWER, separate prerequisite than the note below once said.** `status` lives on `value_object "Keyword"` (`syntax.bluebook:239`) and `value_object "Argument"` (`:581`), and `lifecycle` is admitted only in `context: "Aggregate"` and `context: "Entity"` (`:317`, `:340`) — `ValueObjectBuilder` defines `one_of`, `member` and `invariant` and nothing else. A value object has no identity, so it cannot transition; it is replaced wholesale. `Keyword` and `Argument` must become entities of `Syntax` first — DONE (S17) delivered the general nested-entity machinery this needs, but did NOT itself convert Keyword/Argument: ADR 0026's own text names only Member/Dispatch/Handler, never Keyword/Argument, and S17 was scoped and verified against that text exactly. Converting Keyword/Argument is real, separate, NOT YET SCOPED work — needs its own dedicated research pass before starting (does either hold an open-map shape like Member's own `pairs` did? does `Syntax` already have real top-level records these would collide with?), the same discipline "A caution about this document" (below) asks for and S17 itself needed twice before landing. Treat it as S14's own first step, not an inherited S17 leftover. The value is still real — `bin/model_check` calls the language "clean" because it declares no states at all — it just cannot be had cheaply. |
...
- **S14 against S17** — S14 *depended* on S17 landing general nested-entity capability, which it now has. S14 is still blocked, but on its own separate, not-yet-scoped Keyword/Argument conversion — see S14's own row above.
