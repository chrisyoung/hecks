...
| **S6 — events first-class** | S0a, S0b | **Ruby-side core + Rust parser DONE, 2026-08-27; full corpus migration + declared event-payload block still deferred.** `emits`/`on`/saga-transition triggers all accept a bare constant (`Naming.event_ref`, `ConstShim`) or the old quoted string — deliberately NOT refusing the old spelling, since only command references (not event names) are 100% corpus-migrated. `with:` checking already works via shape-inference from the emitting command (`event_shape_for`/`check_with_spec!` — pre-existing), which turned out to be what the ADR's with:-checking goal actually needed; a separate declared `event` block with its own attributes was judged unnecessary for that but may still be wanted for the ADR's DDD framing — a design call, not decided here. Migrated as proof: `Banking::Account`'s `FreezeAccount`(`emits`)/`ReviewOnFreeze`(`on`) pair, end to end. Remaining: ~100+ corpus `emits`/`on` sites still on the quoted-string spelling; Rust codegen (not just the parser) untouched. Gate: full rspec (2244), Rust parity (68, real cargo build), `cargo build`/`cargo test`, rubocop, model_check, doc_coverage, fuzz — all green. |
...
| **S12 — `projects` and the boundary rule** | S0a, S6, S7, S10, S11 | **Investigated 2026-08-27, not built — the ADR's own scope estimate is stale.** `projects`/`ProjectionAbsent`/`rebuild_sweep` (the "give aggregates a local copy" half) are already fully built and wired — confirmed live, don't rebuild. The boundary-confinement half (delete `References#dereference`, force cross-aggregate `given`/`ensures`/`invariant` reads onto `projects` instead) is NOT done, and its blast radius is bigger than the ADR says: `dereference` has **4 call sites** today (`enforce_givens`, `enforce_ensures`, `enforce_invariants`, `check_entity_invariants`), not the ADR's claimed 2 — aggregate `invariant` (S10, landed the same day) also dereferences, and the ADR predates that construct entirely. Raw dotted-chain counts (`given`/`ensures`/`invariant`: 20/4/44) are NOT the real migration count — most are local nested-value-object walks (`balance.cents.zero?`), not cross-aggregate reads; genuine cross-boundary hits sample at ~10-15 in `given` alone, `ensures`/`invariant` unclassified. **Needs a dedicated pass that starts with per-site classification** (cross-boundary vs. local dotted read) before attempting any migration — comparable in scale to S10's 30-command migration, touching dispatch's hottest path. Two agents have now stopped here rather than force an incomplete migration; that's the correct call each time, not a failure to fix. |
