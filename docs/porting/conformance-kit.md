...
`spec/golden/ir/*.json` (one file per domain — `Pizzas.json`, `Banking.json`, `Bluebook.json`, ...) is
...
`Vocabulary::MutationOp` declarations in `lib/hecksagain/language/bluebook/vocabulary.bluebook` by
