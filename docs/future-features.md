...
8. **`OutboundEvent`.** A durable outbox (`pending → claimed → delivered | failed`) for effect-port edges with idempotent delivery ids and a shared adapter-host protocol. hecks's effect ports drop on restart today. PARTIAL, saga-only: `SagaInterpreter` now checkpoints a pending-dispatch marker (`saga_pending_dispatch.rb`) before a leg's own dispatch runs and clears it after, so a crash mid-leg is surfaced loudly on the next boot (`rehydrate_sagas!`'s `warn_stalled_saga`) instead of silently lost — see `docs/implemented/guides/policies-and-process-managers.md`'s "Nobody had to notice" caveat. That's visibility only, still not this item: no `claimed`/generic effect-port coverage, and deliberately no auto-redrive, since redelivering a dispatch of unknown outcome needs the idempotent delivery ids this item still names as unbuilt.
