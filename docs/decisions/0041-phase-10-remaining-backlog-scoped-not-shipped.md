...
Seven real Phase 10 items shipped this session (ADR 0040, 0042, 0045) — four with a real corpus fixture, three (read-model `authorize`, `multiply`, door-argument type-bridging) on purpose-built ones. `cursor`/`consistency` is investigated and closed (ADR 0044): not a real gap at all. Of the three still genuinely open: `corrects` and `group_by`/`count`/`median` need genuinely new kernel/adapter infrastructure this session did not build (`corrects`'s own mechanical half — the no-op mutation, the allow-list entry — IS small and DOES have real corpus motivation, `Banking::Account.CorrectFee`, but shipping it alone, without the admissibility check that makes `corrects` mean anything, would be worse than not shipping — a `CorrectFee` that silently accepts correcting an event that never happened); `clamp`/`remove` and `list_of admits:/pattern:` are structurally close to existing patterns but have zero real corpus motivation to verify a fix against, and (for `clamp`/`remove` specifically) a genuinely different emission shape than the arithmetic-pair `multiply` just reused — `list_of admits:/pattern:` is now ALSO blocked on the deeper, unresolved correctness question ADR 0043 surfaced. None of the three is a same-shape repeat of what's already shipped — each needs its own dedicated round.
