...
## Update, 2026-08-21 — the executor is production-wired; the LOC metric has a real answer, not just a future check

The gap the 2026-08-20 update named — a real EXECUTOR, with `PolicyInterpreter`/`SagaInterpreter` retired IN ITS FAVOR — is real now, for the "retired in favor of" half specifically: both interpreters lower every reaction and run it through the SAME `ReactionExecutor` instead of their own former hand-rolled dispatch logic (full detail in PRD 12, ADR 0029's own matching update). Both classes themselves still exist — event-matching, `for_each`, instance lifecycle, and compensation orchestration are genuinely construct-specific and stay hand-written — so this is convergence of the shared MECHANISM, not deletion of either interpreter as a class. ADR 0029's own step 5 (a dedicated full-corpus/fuzzer-parity gate, distinct from "the existing suite happens to pass") is what would justify that further step, and hasn't run as its own pass yet.

**The LOC metric (Consequences, below, as originally written) said: checked "once a real production swap is on the table, not mid-scaffolding."** That table is set now, for the mechanism if not yet the classes: `PolicyInterpreter`/`SagaInterpreter` no longer each independently implement refusal/defect/retry rescue, depth-ceiling checking, binding resolution, or checkpoint-boundary/compensation-dispatch logic — one shared `ReactionExecutor` does, called by both. This is the real, checkable form of "fewer irreducible primitives reduce handwritten kernel LOC" the metric asked for, on the Ruby side; the Rust side (below) is unaffected by this update — no Rust representation of `Reaction`/`PrimalIR` exists yet, so there is nothing to measure there until that work starts.

**Naming, since this update:** the executable algebra (`Reaction`/`Trigger`/`CommandRef`/`Dispatch`/`Context`/`Persistence`/`Failure`) is `Hecksagain::PrimalIR` now, a real top-level module (`lib/hecksagain/primal_ir.rb`), not nested under a `Runtime::` lowering concern — this ADR's own "canonical IR" is `Hecksagain::BluebookIR` (renamed, mechanically, codebase-wide, from `Hecksagain::IR`) throughout this document from here on, matching the code.

**A real correction to this ADR's own stated premise, found checking "what does Rust have today" against the real tree instead of repeating what earlier drafts assumed: Rust already has a hand-written reaction engine.** `rust/src/kernel/orchestrate.rs` (981 lines) is a direct, hand-authored port of `PolicyInterpreter#react`+`SagaInterpreter#advance`'s own reaction plumbing — its own header names both Ruby methods as what it ports, function for function (`begin_saga`/`advance_saga`/`deliver_saga_dispatch`/`unwind`/`end_saga`, kept as five separate functions rather than merged, for the same reason Ruby keeps them separate) — differentially verified against Ruby's own `reaction_log`/`saga_log` by `spec/rust_conformance_spec.rb`. This means "Rust's obligations shrink, but only once this is built" (Consequences, below, as originally written) UNDERSTATED the real situation: Rust's reaction obligations are not ZERO today, waiting on this ADR — they're MET TODAY by 981 lines of hand-duplicated logic, kept in sync with Ruby's own interpreters by a human re-transcribing changes, which is exactly the ADR 0022-shaped defect this whole ADR exists to close, one level up from where 0022 itself found it (`expr.rs` vs. `Evaluator`) and one level up again from where ADR 0029's own `Reaction`/`Binding` extraction just closed it in Ruby.

**The real next milestone, precisely scoped rather than left as "Rust representation of Reaction":** converge `orchestrate.rs` onto an executor running a GENERATED `PrimalIR::Reaction` mirror — the SAME kind of `hecks-exec-ir`-consuming kernel this ADR has always described, applied to the one slice (`Reaction`) that turns out to already have a hand-written Rust implementation competing with it. Following `rust/src/kernel/binding/`'s own established precedent (a plain Ruby shape manifest — `Bluebook::Expression::BindingShape` — checked equal to the real Ruby struct definitions by a spec, `bin/project_binding_shape` generating the Rust structure from it, hand-written `logic.rs` alongside): this is real, substantial, well-scoped follow-up work, NOT attempted in this pass, because it needs a real answer to a question this ADR's own proof sequence never had to face for Expression or Binding — HOW does a real, LOWERED `PrimalIR::Reaction` for every actual domain's policies/process managers get INTO Rust at all, without Rust ever reading canonical `BluebookIR`-shaped `Policy`/`ProcessManager` data itself (this ADR's own explicit boundary, "the algorithm that reads Bluebook and emits Reaction lives ONLY in Ruby")? Binding's own answer was simpler — the SHAPE alone (`Literal`/`Reference`) was enough, because nothing in Rust needed real, populated Binding INSTANCES for actual domains, only the node types. `Reaction` is different: `orchestrate.rs` needs real instances, one lowered `PrimalIR::Reaction` per real policy/handler, for every domain it processes — which means this milestone needs either a build-time serialization/embedding step (Ruby lowers, writes out, Rust decodes) or some other real design this pass didn't invent. Replacing 981 already-tested lines is also not a from-scratch build — it needs `spec/rust_conformance_spec.rb`'s own differential coverage held throughout, not just at the end.

## Update, 2026-08-21 (continued) — the stopping rule, made explicit; the lifecycle gap closed; where the real lever actually is

**The Ruby-side measurement is in, and it answers the question the LOC metric was actually trying to ask.** `PolicyInterpreter`/`SagaInterpreter`, read line by line and every method categorized: duplicated semantic algorithms remaining between them is **zero** — condition evaluation, checkpoint behavior, dispatch, retry, and binding resolution each have exactly one implementation, `ReactionExecutor`'s. What's left in each interpreter (111 code-only lines in `PolicyInterpreter`, 163 in `SagaInterpreter`) is not accidental duplication surviving an incomplete extraction — it's genuinely different runtime responsibility: `PolicyInterpreter`'s residual code is overwhelmingly event-matching and `for_each` fan-out; `SagaInterpreter`'s is overwhelmingly instance lifecycle, the mutex boundary, and compensation. That is the natural boundary, not a waypoint — a further merge into one dispatcher (considered and rejected this same pass, under the working name `ReactionRuntime`) would combine two genuinely distinct orchestration concerns into one class that knows more total concepts than either predecessor did, for no real semantic simplification.

**A rule, made explicit so future reductions don't re-litigate this by re-chasing the wrong number:**

> One implementation per irreducible semantic behavior. Orchestration remains separate where the behavior is genuinely different. Do not optimize for class count or LOC at the expense of preserving real semantic boundaries.

This is what the LOC metric (Consequences, below) was always reaching for, stated without the metric's own risk of being gamed: total LOC can rise while this rule is fully satisfied (it did, twice — once when `Reaction`/`ReactionLowering`/`ReactionExecutor` were extracted as new files, once again when `Lifecycle` was added below), and total LOC can fall while this rule is violated (forcing two distinct orchestration concerns into one class would likely show up as fewer total lines while actively making the codebase worse). Duplicated-algorithm count, not LOC, is the number that actually tracks whether an extraction succeeded.

**A real gap this same rule caught, closed in the same pass:** `SagaInterpreter#begin_saga`/`#end_saga` never went through `Reaction`/`ReactionLowering` at all — they compared `event.name` against `pm.starts_on`/`pm.ends_on` directly, and built the initial state from `pm.states.first` directly. The "shared `Reaction` kernel" was silently relying on process-manager PROVENANCE for behavior it claimed to represent explicitly, contradicting this ADR's own Question 6 ("no hidden mode switch"). Closed by adding `PrimalIR::Lifecycle` (`Begin`/`Continue`/`End`, a field on `Context::Correlated`) and a real, previously-missing third `Persistence` strategy, `Ended` (an ended instance's own persisted record is DELETED, not moved to a new state — genuinely doesn't fit `Checkpointed` or `Ephemeral`). Both `begin_saga` and `end_saga` now read their own trigger name (and, for `begin_saga`, initial state) off a real lowered `Reaction` instead of off `pm` directly — the EXISTENCE gating itself (does an instance already exist for this correlation) stays hand-written in `SagaInterpreter`, on purpose: `Reaction` genuinely cannot know the owning process manager's own NAME, the registry's own keying field, only a correlation FIELD name.

**Where the real lever actually is, corrected once more:** not "Expression self-hosting" — that's Slice 1, already done, `kernel/expr` already generated, nothing hand-duplicated there. It's `orchestrate.rs` itself, named two updates above. The next measured step there, BEFORE any rewrite is attempted: classify every one of its 981 lines into exactly three buckets — (A) canonical interpretation/lowering that `Reaction`'s own lowering has already decided and should disappear entirely ("is this a policy," "find the right handler," "derive lifecycle from handler position," "translate bindings," "determine persistence from canonical shape"); (B) irreducible `Reaction` execution semantics that must stay, because Rust genuinely has to execute behavior (evaluate condition, resolve binding, dispatch, classify outcome, retry, checkpoint — some of which may already live in `kernel/expr`/`kernel/binding`/the new `kernel/reaction/logic.rs`); (C) genuinely Rust-target-specific mechanics that stay regardless (host calls, synchronization, the WASM boundary). The size of bucket A, not a guess, is what tells whether this is a ~100-line cleanup or the large Rust deletion this ADR has suspected was hiding in the architecture since the "already has a hand-written reaction engine" finding above. Not measured yet — the next real step, not this one.

**The sharper parity boundary this would buy, once built:** today, `spec/rust_conformance_spec.rb` differentially tests "Ruby orchestration" against "independently implemented Rust orchestration" — useful, but expensive forever, since it can only ever prove the two INDEPENDENT implementations happen to agree today. Moving the equivalence boundary up to the `Reaction` IR itself splits that into two smaller, cheaper claims: lowering correctness (`Policy`/`Handler` → `Reaction`, checked once, in Ruby) and kernel parity (`execute(Reaction)` agrees between Ruby and Rust, checked against the SAME data both sides received, not two sides that each independently decided what a `Policy` means). A genuine architectural test of whether this ADR worked, once done: search Rust's own execution path for `Policy`/`ProcessManager`/`Saga` — the runtime kernel ideally shouldn't need those words at all, only `Reaction`/`Context`/`Persistence`/`Lifecycle`/`Failure`/`Binding`/`Expression`.

## Update, 2026-08-21 (continued) — `orchestrate.rs`, classified: measured evidence instead of a guess

The classification named above ran — every one of `orchestrate.rs`'s ~880 production lines (its own `#[cfg(test)]` module, ~95 more lines, is out of scope for this question), read directly and sorted into the three buckets, plus a fourth this audit itself needed to add.

**What provably disappears (bucket A, ~230 lines, ~26%).** `PolicyRule`/`CrossDomainPolicyRule`/`Handler`/`ProcessManagerDef` (lines 74–183, ~85 lines) are Rust's own hand-maintained mirror of canonical `Policy`/`ProcessManager` shape — exactly what `ReactionLowering` already reduces away in Ruby. `correlation_head` (206–208) is a pure derivation from `correlates_by`'s dotted path, already baked into `Context::Correlated.correlation_key` at lowering time. Trigger/handler-name matching scattered through `react_policies` (435–443), `begin_saga` (581), `advance_saga` (631), `end_saga` (794) — "does this event name match `pm.starts_on`/`pm.ends_on`/`handler.event_type`" — is exactly `Trigger.name` comparison, now that `Lifecycle::Begin`/`Continue`/`End` make every stage explicit as data. None of this needs to exist once Rust receives an already-lowered `Reaction`.

**What converges to functions that already exist, unwired (bucket B, ~435 lines, ~49% — but roughly half of THAT is not "irreducible," it's already duplicated).** `resolve_with`/`build_dispatch_args`/`trigger_args` (268–288, 385–415, ~70 lines) are a hand-written reimplementation of exactly what `kernel::binding::resolve` and this same slice's own `kernel::reaction::logic::resolve_dispatch_bindings` already do generically — including the VERBATIM/empty-bindings distinction (`trigger_args`'s own "empty `with_spec` forwards the whole payload" IS `Bindings::Verbatim`). Not "irreducible logic that happens to look similar" — the same algorithm, twice, one hand-written and one already generated and tested. The rest of bucket B — depth-ceiling checks (737), command qualification (757–761), the from-state check (652), checkpoint-then-dispatch sequencing across `advance_saga`/`deliver_saga_dispatch`/`compensate` (~300 lines) — is the same shape `ReactionExecutor`/`match_and_checkpoint?`/`dispatch_one!` already implement in Ruby, reimplemented by hand rather than calling a shared kernel function once one exists in Rust.

**What necessarily remains (bucket C, ~85 lines, ~10%, plus a real fourth category, ~90 lines, ~10%, that this audit is what surfaced).** `Tables` (295–303, thread-N-args-as-one-struct ergonomics), the recursive reentry/depth-tracking/event-accumulation core of `orchestrate` itself (326–372), and `WithValue::Literal(fn() -> Json)`'s function-pointer trick (`Json` isn't const-constructible) are genuinely Rust-target-specific and stay regardless. Separately — not Rust mechanics, not canonical duplication, not shared semantics — `for_each`'s query-run-and-per-row-fan-out (472–521) and cross-domain policy matching/queueing (549–571) are genuinely distinct capabilities with no `Reaction` equivalent, matching `for_each`'s own status on the Ruby side (PRD 12's own non-goal).

**Two real gaps this audit found in `PrimalIR` itself, more important than the LOC count:**

- `correlation_of`'s (218–259) three-tier fallback is a genuine per-EVENT algorithm — not something lowering can decide ahead of time — but its tier 1 needs the RAW `correlates_by` dotted path, which `Context::Correlated` doesn't currently carry, only the already-split `correlation_key` (the head). `Context::Correlated` is under-specified for what the runtime actually needs to reproduce this algorithm generically, not merely unwired.
- Cross-domain policy dispatch (`CrossDomainPolicyRule`/`PendingCrossDomainReaction`) has no representation in `CommandRef`/`Reaction` at all — the shape has no way to say "this dispatch targets an external domain, requiring host-mediated delivery, not a local `dispatch_fn` call." This is a real incompleteness in the executable algebra, not a Rust-side implementation detail — closing it means widening `Reaction`'s own shape, the same kind of change `Lifecycle`/`Persistence::Ended` just were, not a mechanical port.

**The honest estimate, held as a range, contingent on closing both gaps above without introducing equivalent complexity elsewhere:** current evidence supports reducing `orchestrate.rs` from ~880 production LOC to roughly **450–550 handwritten LOC** — not the ~300–400 an optimistic guess floated, and not "only ~100 lines are actually redundant" either. About half of the file is not really Rust-specific orchestration at all; it's either canonical interpretation that should already have been lowered away, or duplicate execution semantics that should already live in the shared kernel. That is the confirmation this ADR's own "already has a hand-written reaction engine" finding, above, was pointing toward — strong enough to justify the next slice as real, scoped work, not strong enough to justify skipping the two gap-closing design questions first.
...
- **The LOC metric, sharpened, not abandoned.** PRD 09's own "semantic implementation count, not evaluator line count" is correct for judging any ONE slice in isolation — Expression/Binding/Reaction each necessarily add generated scaffolding (and, for Reaction, a whole new executor) before anything old is deleted, so total repository LOC rising mid-sequence is expected, not a warning sign on its own. But that framing must not become a permanent excuse: **fewer irreducible primitives should reduce handwritten KERNEL LOC, even as generated LOC rises** — 500 handwritten lines becoming 250 handwritten + 400 generated is the healthy shape PRD 09's metric describes; 500 becoming 900 handwritten is not, whatever the semantic-primitive count says. The real payoff this whole sequence is for is deleting whole bespoke semantic paths outright — `PolicyInterpreter`, `SagaInterpreter`, the duplicated `Expr`/`Evaluator` grammar, duplicated validators/serializers, duplicated Rust match arms — not adding a fourth layer beside them. Concretely, for Rust specifically: handwritten kernel LOC is now an explicit tracked metric alongside semantic-implementation count, checked once `Reaction`/`Binding` convergence is far enough along to matter (i.e., once a real production swap is on the table, not mid-scaffolding) — if handwritten Rust LOC hasn't dropped by then, that's real evidence the abstraction isn't paying for itself, not something the semantic-count metric gets to wave away.
