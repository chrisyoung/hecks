...
- Walks `docs/guides/*.md` **and README.md**; one `it` per file, one shared
...
- Output: `docs/reference/<context>.md` (one page per context: aggregate,
...
- Golden spec: render in-memory, byte-compare whole committed files;
  ...
  "the language and docs/reference/X disagree — run bin/reference, review the diff."
...
- Same marker mechanism as reference pages; `bin/reference` (or a sibling
  ...
  from docs/guides/*.md), reference index (contexts), bin/ tool table (first
...
## Guides to write — docs/guides/ (all doctested unless noted)
...
Plus `docs/guides/AUTHORING.md` (not doctested): harness conventions for future
