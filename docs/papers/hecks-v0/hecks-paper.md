...
version: "paper/hecks-v0-2026-04-23"
commit: "e33c6672"
date: 2026-04-23
---
...
**Version:** `paper/hecks-v0-2026-04-23`
**Repository commit at time of writing:** `e33c6672`
**Authors:** Chris Young, with contributions from *(placeholder — author list to be filled in by Chris Young prior to deposit).*

## Abstract

Hecks is an open-source domain compiler that treats a software domain as a first-class artifact described by five Domain-Specific Languages (DSLs): `.bluebook` for the domain model, `.hecksagon` for hexagonal ports and adapters, `.fixtures` for seed data and catalog schemas, `.behaviors` for behavioural tests, and `.world` for runtime and extension configuration. The primary runtime is a Ruby interpreter (`lib/`) that boots a domain at load time, registers a command bus, and exposes an event-sourced aggregate model. A secondary runtime written in Rust (`hecks_life/`) is maintained byte-identically to the Ruby runtime through a hand-written canonical Intermediate Representation (IR) and a 920-fixture parser parity corpus. Hecks then ships sixteen data contracts (`lib/hecks/conventions/`) that drive multi-target code generation to Go, Node/TypeScript, and a zero-dependency Ruby static target. The framework also describes itself: thirteen "chapter" bluebooks enumerate every Ruby class in `lib/`, a coverage verifier asserts they match, and a parity verifier confirms the Ruby and Rust interpreters agree on aggregate and command counts per chapter. Two self-hosting arcs are reported. First, `hecks compile` produces a single-file zero-dependency Ruby binary of the framework by composing an Abstract Syntax Tree (AST) analysis with a Bluebook-IR method-call graph — an instance of Futamura's first projection (Futamura, 1971) applied to the Ruby interpreter. Second, an in-progress arc ("i51") applies partial evaluation to the Rust runtime: each hand-written Rust module is described as a shape-only bluebook, lowered through an L0–L8 IR factoring, and regenerated byte-identically via a shell-adapter-dispatched specializer; Phase A retires `validator.rs` with a byte-identical generated file. We enumerate twenty-plus techniques with direct file references as the defensive-publication payload. The paper's claims are reproducible from the public repository at the tag above.
...
The paper has two audiences. For the enterprise developer, we describe the DSL vocabulary, the hexagonal-adapter declaration mechanism, and the behavioural-test cascade lockdown with the intent that they be immediately recognisable and adoptable. For the programming-language and compiler-research audience, we describe the framework's self-hosting properties — particularly the factoring of the Rust interpreter into eight IR layers (L0–L8) and the module-by-module retirement of hand-written Rust via partial evaluation. The latter work is in flight; what has shipped is a byte-identical regeneration of `hecks_life/src/validator.rs` dispatched through a domain-level shell adapter. This is framed formally in §9 as an instance of Futamura's first projection.
...
8. An in-flight module-by-module Futamura specialisation of the Rust runtime in which the specialiser is wired as a hexagonal shell adapter, so the codegen pipeline is dispatchable as a domain command rather than invoked as a build script.
...
A `.hecksagon` file declares the ports and adapters for a domain. Adapters are not imported as libraries; they are *named and configured* in a DSL that Ruby and Rust runtimes both parse.
...
- `e33c6672` — retire hand-written `validator.rs`; the file now carries a GENERATED FILE header citing `bin/specialize-validator --output hecks_life/src/validator.rs`. Integration tests move to `hecks_life/tests/validator_rules_test.rs` to break the circular dependency between validator and its own tests.
...
A novel aspect of Hecks's Futamura arc: the specialiser is wired as a hexagonal shell adapter rather than invoked as a build script. `specializer.hecksagon` (quoted in §5.1) declares `adapter :shell, name: :specialize_validator, command: "bin/specialize-validator", args: ["--output", "{{output}}"]` and gates the resulting `Specialize` command behind an `:autophagy` gate. The implementation behind the shell adapter is Ruby today (`bin/specialize-validator`); Phase B will flip the command string to a `hecks-life` subcommand with no other file changes.

### §9.7 Phase B — Module-by-module retirement (plan)

Phase B retires additional Rust modules, ordered declarative-first so the partial evaluation surface grows monotonically:

1. `validator.rs` and `validator_warnings.rs` (Phase A).
2. `fixtures_parser.rs`, `behaviors_parser.rs`, `hecksagon_parser.rs`.
3. `dump.rs` (the canonicalizer — almost declarative).
...
### §9.8 Phase C — Second Futamura projection (planned)

Phase C lifts the specialiser into its own bluebook and applies it to itself, producing $\mathtt{compiler} = \mathtt{mix}(\mathtt{mix}, I)$. The success criterion is the fixed point $\text{binary}_N \equiv \text{binary}_{N+1}$. When step 3 of this fixed-point loop holds, the system is strictly self-hosting in the sense of §9.10 below.

### §9.9 Phase D — Third Futamura projection (deferred)

$\mathtt{mix}(\mathtt{mix}, \mathtt{mix}) = \mathtt{compiler\_generator}$. Useful for generating compilers to additional targets (Go, WebAssembly) without rewriting the specialiser. Out of i51 scope.

### §9.10 Two bars to distinguish
...
### §10.1 Test-suite numbers (at `e33c6672`)

**Table 10.1 — Test suite at commit e33c6672.**

| Suite                          | Result                      | Runtime |
|--------------------------------|-----------------------------|---------|
| `rspec spec/` (Ruby)           | 130 examples, 0 failures    | 1.23 s  |
| `cargo test --release` (Rust)  | 204 passed, 0 failed, 22 integration test files | < 1 s |
| Parity overall                 | 497 / 504 pass              | ~1 s    |
| Parity hard                    | 129 / 129 pass              |         |
| Parity nursery (soft)          | 368 / 375 pass              |         |
| `hecks verify` (six phases)    | clean                       | 0.27 s  |

The pre-commit hook runs the parity suite; file-size and test-speed limits are enforced by hooks.

### §10.2 Corpus

**Table 10.2 — DSL corpus at e33c6672.**
...
The Ruby parser has two open soft-drift items (inbox i1 and i2) in which certain Symbol-to-Float and Symbol-to-Integer conversions diverge from the Rust parser. These account for seven nursery fixtures. The user is resolving these ahead of the paper's deposit; the paper assumes `hecks verify` is clean.

The Rust runtime does not yet cover every runtime capability of the Ruby runtime. Notably, the HTTP server path (`hecks serve`) is Ruby-only at time of writing; the Rust binary focuses on parsing, validation, `.heki` I/O, and the agent daemons used by Miette.

Futamura Phase B is not complete. The retirement plan in §9.7 is an ordering hypothesis, not a shipped artifact.

The chapter self-description (§7, §13.4) is accurate by discipline, not by construction. The `CoverageVerifier` matches on file basename rather than public-method signature, so a chapter aggregate may declare a stale command set while the coverage phase continues to pass. A semantic reconciler that walks the Ruby AST and diffs public methods against chapter commands is planned but not shipped.

### §14.2 Future work

- Complete Phase B module-by-module retirement (§9.7).
- Phase C second-Futamura fixed point (§9.8): $\text{binary}_N \equiv \text{binary}_{N+1}$.
- Additional code-generation targets: WebAssembly, Kotlin, Python.
- Deterministic codegen infrastructure (stable sort orders, frozen timestamps) sufficient for Phase C success.

---

## §15 Conclusion

Hecks is a domain compiler that collapses the distinction between the domain model and the code. Five DSLs declare the domain, its adapters, its seed data, its tests, and its runtime configuration. Sixteen data contracts route every code-generation decision through a single type registry. Two runtimes — Ruby and Rust — agree on a canonical JSON IR, a parity test suite, and a chapter self-description. A shipped binary compiler and an in-progress module-by-module Futamura specialisation apply partial-evaluation techniques to the framework itself. We publish these techniques with direct file references at tag `paper/hecks-v0-2026-04-23` as defensive prior art.
